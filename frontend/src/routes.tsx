...
// ✅ Nuevo: Bloquea a los admin (solo permite no-admin)
function RequireNonAdmin({ children }: { children: React.ReactNode }) {
  const { token, role } = getAuth();
  if (!token) return <Navigate to="/login" replace />;
  if (role === 'admin') return <Navigate to="/admin" replace />; // o "/" si prefieres
  return <>{children}</>;
}
...
export const router = createBrowserRouter([
  {
    ...
    children: [
      ...
      // rutas de usuario (bloquea admin)
      {
        path: 'main', element: (<RequireUser><RequireNonAdmin><MainPage /></RequireNonAdmin></RequireUser>), },
      {path: 'history', element: (<RequireUser><RequireNonAdmin><HistoryRoute /></RequireNonAdmin></RequireUser>), },
